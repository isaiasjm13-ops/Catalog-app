...
$sqlPath = Join-Path $PSScriptRoot 'apply_followup_migration.sql'
$resultPath = Join-Path $env:TEMP 'perfect_catalog_0002.exit'

if (-not (Test-Path -LiteralPath $psqlPath)) {
    throw "psql no existe en la ruta esperada: $psqlPath"
}

Write-Host 'Aplicando migracion 0002 en perfect_catalog_dev.'
