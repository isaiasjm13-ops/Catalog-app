...
$sqlPath = Join-Path $PSScriptRoot 'apply_brand_profiles_migration.sql'
$resultPath = Join-Path $env:LOCALAPPDATA 'Temp\perfect_catalog_0013.exit'
if (Test-Path -LiteralPath $resultPath) { Remove-Item -LiteralPath $resultPath -Force }
if (-not (Test-Path -LiteralPath $psqlPath)) { throw "psql no existe en la ruta esperada: $psqlPath" }
Write-Host 'Aplicando migracion 0013 de perfiles de marca en perfect_catalog_dev.'
