...
$sqlPath = Join-Path $PSScriptRoot 'apply_image_archive_index_migration.sql'
$resultPath = Join-Path $env:LOCALAPPDATA 'Temp\perfect_catalog_0009.exit'
if (Test-Path -LiteralPath $resultPath) { Remove-Item -LiteralPath $resultPath -Force }
if (-not (Test-Path -LiteralPath $psqlPath)) { throw "psql no existe en la ruta esperada: $psqlPath" }
Write-Host 'Aplicando migracion 0009 en perfect_catalog_dev.'
