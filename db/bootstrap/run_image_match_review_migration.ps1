...
$sqlPath = Join-Path $PSScriptRoot 'apply_image_match_review_migration.sql'
$resultPath = Join-Path $env:LOCALAPPDATA 'Temp\perfect_catalog_0010.exit'
if (Test-Path -LiteralPath $resultPath) { Remove-Item -LiteralPath $resultPath -Force }
if (-not (Test-Path -LiteralPath $psqlPath)) { throw "psql no existe en la ruta esperada: $psqlPath" }
Write-Host 'Aplicando migracion 0010 en perfect_catalog_dev.'
