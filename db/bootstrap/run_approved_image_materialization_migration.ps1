...
$sqlPath = Join-Path $PSScriptRoot 'apply_approved_image_materialization_migration.sql'
$resultPath = Join-Path $env:LOCALAPPDATA 'Temp\perfect_catalog_0011.exit'
if (Test-Path -LiteralPath $resultPath) { Remove-Item -LiteralPath $resultPath -Force }
if (-not (Test-Path -LiteralPath $psqlPath)) { throw "psql no existe en la ruta esperada: $psqlPath" }
Write-Host 'Aplicando migracion 0011 en perfect_catalog_dev.'
